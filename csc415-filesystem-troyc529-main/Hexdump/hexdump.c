...
int processFile (char * filename, uint64_t startBlock, uint64_t numBlocks)
	...
	if (position > endOfFile)		//can not start past the end of the filename
		{
		printf ("Can not dum5/csc415-filesystem-JohnsonNguyen-Py/Hexdump$ gcc hexdump.c -o hexdumpgcc hexdump.c -o hexdumpp file %s, starting at block %llu, past the end of the file.\n\n", 
